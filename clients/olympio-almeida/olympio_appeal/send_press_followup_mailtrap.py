"""
Day 3 follow-up: Goa press contacts via Mailtrap live SMTP.
219 contacts, BCC batches of 45 + Gautam full report.
Run from repo root: python clients/olympio-almeida/olympio_appeal/send_press_followup_mailtrap.py
"""
...
print(f"Goa press contacts (excluding bounces): {len(goa_press)}")
...
night/day limit under the Noise Pollution Rules 2000.
...
The following authorities have been formally invited by letter:
  • Goa State Pollution Control Board (GSPCB) — Calibrated-meter sound measurement requested
...
# ── Mailtrap send function ────────────────────────────────────────────────────
def mailtrap_send(msg):
    ctx = ssl.create_default_context()
    with smtplib.SMTP(MT_HOST, MT_PORT, timeout=30) as s:
        s.ehlo()
        s.starttls(context=ctx)
        s.login(MT_USER, MT_PASS)
        s.send_message(msg)

# ── BCC batching ──────────────────────────────────────────────────────────────
BATCH_SIZE = 45
batches = [goa_press[i:i+BATCH_SIZE] for i in range(0, len(goa_press), BATCH_SIZE)]
print(f"Batches: {len(batches)} × up to {BATCH_SIZE}")
...
for idx, batch in enumerate(batches, 1):
    ...
    all_rcpt = [FROM_ADDR, CC_INFO] + bcc_emails
    try:
        mailtrap_send(msg)
        print(f"  Batch {idx}/{len(batches)}: {len(batch)} press — OK")
        ok += 1
...
print(f"\nPress follow-up: {ok}/{len(batches)} batches sent via Mailtrap")
print(f"Total press recipients: {len(goa_press)}")
...
6. DAY 3 PRESS FOLLOW-UP (MAILTRAP)
──────────────────────────────────────
Today (13 June), sent a second press follow-up to all 219 Goa press/civic
contacts via Mailtrap reminding them of:
...
════════════════════════════════════════════════════════════════
  ...
  TODAY   Sun 15 June  → Press + residents visit Gaunsawaddo (if they come)
                         YOUR TASK (see below)
...
10 AM and 2 PM with the following:
  ...
  2. Capture the meter reading clearly — aim for a reading when courts are active
  3. Each video should be timestamped (your phone's built-in timestamp is fine)
...
Send me the videos by Sunday evening at the latest — we will prepare a
summary note to accompany them.
...
════════════════════════════════════════════════════════════════
  ...
  What to bring:
    □ A copy of your original complaint (March 2026)
...
════════════════════════════════════════════════════════════════
CAMPAIGN STATUS SUMMARY (as of 13 June 2026)
════════════════════════════════════════════════════════════════

  Officials formally notified:   ~69 (55 original + 14 new)
  Press contacts reached:        ~219 Goa press (3 waves)
  Civic organisations:           9
  Calendar invitations:          ~247 parties (2 waves)
...
The campaign has strong momentum. The key remaining task is ensuring
the inspection on 17 June produces a written report and that GSPCB
commits to a Sunday measurement.
...
try:
    mailtrap_send(gau_msg)
    print(f"\nGautam report — OK → {GAUTAM_TO}")
except Exception as e:
    print(f"\nGautam report — FAILED: {e}")
