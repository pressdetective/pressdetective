...
function cell(children, width, fill) {
  const kids = Array.isArray(children) ? children : [new Paragraph({ children: [new TextRun({ text: String(children), size: 18 })] })];
  const opts = { borders, width: { size: width, type: WidthType.DXA }, margins: cellMargins, children: kids };
  if (fill) opts.shading = { fill, type: ShadingType.CLEAR };
  return new TableCell(opts);
}
function txtCell(text, width, fill, color) {
  return cell([new Paragraph({ children: [new TextRun({ text, size: 18, color })] })], width, fill);
}
function sevColor(s){ return s==="High"?RED : s==="Medium"?AMBER : GREEN; }

const invHeader = new TableRow({ tableHeader: true, children: [
  headerCell("Source / Property", 2600),
  headerCell("What it says", 3300),
  headerCell("Severity", 1100),
  headerCell("Visibility", 1100),
  headerCell("Removability", 1260),
]});
const invRows = [
  ["Wikipedia — “Viveka Babajee”", "Names Vora as her boyfriend; quotes the diary line “you killed me, Gautam”. High-authority, permanent, top-ranking.", "High","High","Very low"],
  ["Grokipedia — “Viveka Babajee”", "AI-generated encyclopedia entry repeating the relationship and the note. Rising visibility.", "High","Medium","Low"],
  ["Yahoo News — “…boyfriend arrested in Tikku murder”", "Headline fuses both stories: ties the Babajee relationship to a murder arrest. Most damaging framing.", "High","Medium","Low"],
  ["DNA India (multiple)", "Mix: “clean chit to Gautam Vora” (favourable); “family says she was in a relationship”; “HC grants bail… Tikku murder”.", "High","High","Low"],
  ["Business Standard — “Court rejects bail of Gautam Vora”", "Reports sessions-court bail rejection in the Palande/Tikku harbouring case.", "High","Medium","Low"],
  ["Deccan Herald — “clean chit to Vora”", "Favourable: foul play ruled out, police clean chit (Aug 2010).", "Low","Medium","n/a (asset)"],
  ["Indian Express archive (2010)", "“Viveka was not depressed, had matched kundali with Gautam.” Archived/premium.", "Medium","Low","Low"],
...
const inventoryTable = new Table({
  ...
  columnWidths: [2600, 3300, 1100, 1100, 1260],
  rows: [invHeader, ...invRows.map(r => new TableRow({ children: [
    cell([new Paragraph({ children: [new TextRun({ text: r[0], size: 18, bold: true })] })], 2600),
    txtCell(r[1], 3300),
    txtCell(r[2], 1100, undefined, sevColor(r[2])),
    txtCell(r[3], 1100),
    txtCell(r[4], 1260),
  ]}))],
});

const actHeader = new TableRow({ tableHeader: true, children: [
  headerCell("Priority", 900),
  headerCell("Action", 4400),
  headerCell("Type", 1560),
  headerCell("Horizon", 1300),
  headerCell("Lead", 1200),
]});
const actRows = [
  ["P1","Stand up an optimized professional identity (personal site + bio, refreshed LinkedIn, verified social handles) under “Gautam Vora” so current, accurate pages exist to rank.","Suppression","0–4 weeks","Press Detective"],
  ...
  ["P2","File targeted Google “Results about you” / Right-to-be-Forgotten removal requests on the most damaging URLs, leading with the police clean chit and absence of conviction.","Removal","2–8 weeks","Press Detective + client"],
...
const actionTable = new Table({
  width: { size: 9360, type: WidthType.DXA },
  columnWidths: [900, 4400, 1560, 1300, 1200],
  rows: [actHeader, ...actRows.map(r => new TableRow({ children: [
...
function srcLine(label, url) {
  return new Paragraph({ numbering: { reference: "numbers", level: 0 }, spacing: { after: 40 }, children: [
  ...
  styles: {
    ...
    paragraphStyles: [
      ...
      { id: "Heading2", name: "Heading 2", basedOn: "Normal", next: "Normal", quickFormat: true,
        run: { size: 23, bold: true, font: "Arial", color: GREY },
  ...
  sections: [{
    ...
    children: [
      ...
      new Paragraph({ spacing: { after: 240 }, children: [
        ...
        new TextRun({ text: "•  8 June 2026  •  Confidential", size: 18, color: GREY }),
      ]}),
      ...
      h1("2. The two storylines (the facts on record)"),
      h2("A. Viveka Babajee (2010)"),
      p("Viveka Babajee, a Mauritian model (Miss Mauritius World 1993), was found dead at her Bandra, Mumbai residence on 25 June 2010. A note / diary entry reading “you killed me, Gautam” was recovered. The client, described in coverage as her boyfriend and a stockbroker / stock analyst, was initially under scrutiny for abetment alongside an event manager. In August 2010 police closed the investigation: forensic analysis ruled out foul play and a clean chit was issued. No charge was sustained."),
      h2("B. Arun Tikku murder (2012)"),
      p("In 2012 the client was arrested for allegedly sheltering Vijay Palande — the prime accused in the murders of Arun Tikku and Karan Kakkad — including driving him, helping him check into a hotel and using a credit card to buy clothes. He was booked under sections 212, 202 and 225 of the IPC (harbouring an offender and related). A sessions court initially rejected bail; the Bombay High Court subsequently granted it. The final disposition of his role is not clearly documented in public sources — a gap worth closing, because an exoneration, discharge or acquittal would be a powerful and legitimate counter-fact."),
      ...
      inventoryTable,
      new Paragraph({ spacing: { before: 100 }, children: [new TextRun({ text: "Note: the Deccan Herald and DNA “clean chit” reports are assets, not liabilities — they should be amplified, not buried.", size: 18, italics: true, color: GREEN })] }),

      new Paragraph({ children: [new PageBreak()] }),
      h1("4. Strategy: removal vs. suppression"),
      h2("What can realistically be removed"),
      bullet("News archives (DNA, Deccan Herald, Indian Express, Business Standard, Yahoo): publishers very rarely unpublish accurate reporting. Removal is feasible only via a Google de-indexing / Right-to-be-Forgotten request or a court order. India has no RTBF statute, but several High Courts have ordered de-indexing of dated criminal coverage, particularly where the person was acquitted or never convicted — which is why confirming the Tikku outcome matters."),
      bullet("Wikipedia & Grokipedia: a sourced mention cannot be deleted on request. The achievable goal is accuracy and neutrality — ensuring the clean chit and no-conviction status are present and the tone is not prejudicial. On Wikipedia this must go through the Talk page with the conflict of interest openly disclosed; never edit the live article on the client’s behalf."),
      bullet("anujtikku.com: this is the victim’s family’s own site. Removal is highly unlikely and any approach would be insensitive and counter-productive. Leave it; out-rank it instead."),
      ...
      h2("What we suppress (the core of the plan)"),
      ...
      p("Because the name “Gautam Vora” is shared by many people, a strong, distinct professional identity also helps Google disambiguate him away from the model / murder coverage."),

      h1("5. Prioritized action plan"),
      actionTable,

      h1("6. Guardrails — what we will not do"),
      p("These protect the client far more than they constrain him; reputation campaigns that ignore them tend to backfire publicly."),
      ...
      bullet("No “guaranteed removal” vendors or bogus DMCA / copyright claims to force takedowns — these are scams or legal exposure."),
      ...
      new Paragraph({ spacing: { before: 200 }, children: [new TextRun({ text: "This audit reflects publicly visible search results as of 8 June 2026 and is a strategic plan, not legal advice. Removal of court and news records may require qualified Indian legal counsel.", size: 16, italics: true, color: GREY })] }),
