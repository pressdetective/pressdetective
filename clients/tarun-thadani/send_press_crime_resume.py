...
Account rotation: batches 1-3 â†’ santosh, 4-6 â†’ info, 7-8 â†’ sujata
...
# Use Postmark SMTP â€” no per-hour rate limits unlike ProtonMail Bridge
...
BATCH_SIZE            = 47    # 47 BCC + 3 CC = 50 total â€” Postmark's per-message limit
...
def send_report(total, sent_ok, failed_count):
    ...
    report_body = (
        f"PRESS CAMPAIGN RESUME REPORT â€” {datetime.datetime.now().strftime('%Y-%m-%d %H:%M')}\n"
    ...
    msg['Subject'] = f'[REPORT] Press campaign resume â€” {sent_ok}/{total} delivered'
    ...
    try:
        ...
        print(f'\nReport sent â†’ {CC_ALWAYS}, {CC_COACCUSED}')
    except Exception as e:
...
def main():
    ...
    print('=' * 65)
    print('PRESS CAMPAIGN RESUME â€” FIR 0654/2022 / TARUN THADANI')
    print('=' * 65)
    ...
    if not targets:
        print('Nothing to send â€” all contacts already delivered.')
        return
    ...
    for i, batch in enumerate(batches):
        ...
        if status == 'sent':
            ...
            print(f'  OK â€” batch {i+1} sent to {len(batch)} contacts')
            for row in batch:
                log_rows.append({
