...
Usage: python send_audit.py   (reads .creds for ZEPTO_TOKEN)
"""
...
# Load token from .creds file if not already in env
if not os.environ.get("ZEPTO_TOKEN") and CREDS.exists():
    for line in CREDS.read_text().splitlines():
        if line.startswith("ZEPTO_TOKEN="):
            os.environ["ZEPTO_TOKEN"] = line.split("=", 1)[1].strip()

TOKEN     = os.environ.get("ZEPTO_TOKEN", "")
FROM_ADDR = "sujata.shirasi@pressdetective.com"
TO_ADDR   = "tarun@dharte.com"
CC_ADDRS  = ["tonymony@gmail.com"]
...
ABHISHEK SARAF'S DOCUMENTED RECORD ONLINE (works in your favour)
  • Change.org petition (181 signatures): "Justice for Jamsetji Tata's Home —
    Reclaiming the Third Floor from Abhishek Saraf." Documents the ₹150 crore
    Esplanade House property fraud, forged power of attorney, parallel bank
    account to divert rent. (change.org/p/justice-for-jamsetjis-tata...)
...
def send():
    if not TOKEN:
        raise SystemExit("ERROR: ZEPTO_TOKEN not found in env or .creds file")
    m = EmailMessage()
    m["From"]    = FROM_ADDR
    m["To"]      = TO_ADDR
    m["Cc"]      = ", ".join(CC_ADDRS)
